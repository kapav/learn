    ...
    $.gevent = (function () {
        ...
        publishEvent = function () {
            var argList = [],
              ...
              event_obj, data, dataList;

            argList = argList.slice.call(arguments, 0);
            arg_count = argList.length;

            if (arg_count === 0) { return false; }

            event_name = argList.shift();
            ...
            event_obj.trigger(event_name, dataList);
            return true;
        };
        ...
        // Returns  : none
        //
        subscribeEvent = function ($collection, eventName, fn) {
            $collection.on(eventName, fn);

            if ($customSubMap[eventName]) {
                $customSubMap[eventName]
                  = $customSubMap[eventName].add($collection);
            }
            else {
        ...
        // Returns  : none
        //
        unsubscribeEvent = function ($collection, eventName) {
            if (!$customSubMap[eventName]) { return false; }

            $customSubMap[eventName]
              = $customSubMap[eventName].not($collection);
